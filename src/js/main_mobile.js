...
$(`#result`).on('click', ()=>{
  ...
  if(scorePoint <= -40000){
    ...
    $(`#rank`).html('Rank-1 悪魔');
    $(`#comment`).html('どうやらあなたはよほどのもの好きのようですね。すでに地獄には行かれましたでしょうか？まだでしたらスマイルくんを最初に20回殴りましょう。もちろんグーで。')
  }
    ...
    rank = 'null'
      ...
      $(`#rank`).html('Rank null');
      if(angelMode != 3 & hellMode <20){$(`#comment`).html('この画面を見ている人がいるとは信じられません。人の手で到達できるとは。本当におめでとうございます！');}
...
let angelMode = 0;
$(`#imgBox`).on('click', ()=>{
 if(isPlaying === 0 || isPlaying === -1 & hellMode<20){
